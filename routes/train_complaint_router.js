...
train_complaint_router.get('/api/get_train_complaint', async function (req, res) {
    try {
        const complaint_by_departname = await train_complaint.find();
        if (complaint_by_departname.length == 0) {
            return res.status(404).json({
                'msg': 'no complaint found for this department'
            })
...
train_complaint_router.get('/api/change_status/:id', async function (req, res) {
    try {
        const complaint_to_change = await train_complaint.updateOne({ _id: req.params.id }, { status: "Resolved" })
        .then(() => {
